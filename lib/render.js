...
function calculateMetatile(options) {
    ...
    var unit = res * options.tileSize;

    // Generate all tile coordinates that are within the metatile.
    var tiles = [];
    for (var dx = 0; dx < metaWidth; dx++) {
        for (var dy = 0; dy < metaHeight; dy++) {
            tiles.push([ z, res, xmin + dx * unit, ymin + dy * unit ]);
        }
    }

    var minx = xmin;
    var miny = ymin;
    var maxx = xmin + metaWidth * unit;
    var maxy = ymin + metaHeight * unit;
    return {
...
function encodeSingleTile(source, image, options, meta, callback) {
    var tiles = {};
    var key = [options.format, options.z, meta.res, meta.xmin, meta.ymin].join(',');
...
MapnikSource.prototype._renderMetatile = function(options, callback) {
    ...
    // Calculate bbox from (z, res, xmin, ymin), respecting metatile settings.
