...
MapnikSource.registerProtocols = function(tilesource) {
    tilesource.protocols['mapnik:'] = MapnikSource;
};
...
MapnikSource.prototype._populateInfo = function(callback) {
    ...
    this._pool.acquire(function(err, map) {
        ...
        if (!info.bounds || info.bounds.length !== 4)
            info.bounds = [ -20037508.34, -20037508.34, 20037508.34, 20037508.34 ];
...
MapnikSource.prototype.getTile = function(z, res, xmin, ymin, callback) {
    z = +z; res = +res; xmin = +xmin; ymin = +ymin;
    if (isNaN(z) || isNaN(res) || isNaN(xmin) || isNaN(ymin)) {
        return callback(new Error('Invalid input: '+z+'/'+res+'/'+xmin+'/'+ymin));
    }

    var format = (this._info && this._info.format) || 'png';
    var key = [format, z, res, xmin, ymin].join(',');
...
MapnikSource.prototype.getGrid = function(z, res, xmin, ymin, callback) {
    z = +z; res = +res; xmin = +xmin; ymin = +ymin;
    if (isNaN(z) || isNaN(res) || isNaN(xmin) || isNaN(ymin)) {
        return callback(new Error('Invalid input: '+z+'/'+res+'/'+xmin+'/'+ymin));
    }
    ...
    var key = ['utf', z, res, xmin, ymin].join(',');
